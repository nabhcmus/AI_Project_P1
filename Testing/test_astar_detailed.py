"""
A* Algorithm Comprehensive Testing & Visualization
Tests A* algorithm 1000 times and generates detailed statistics with visualizations
"""
...
def test_astar_single(target_word=None):
    """Test A* algorithm for a single word"""
    try:
        word_api = TestWordAPI(5, target_word)
        solver = AStarSolver(word_api)
        
        start_time = time.time()
        result = solver.solve()  # Returns a list of guesses
        elapsed = time.time() - start_time
        
        # A* solver returns list of guesses, not a dict
...
def run_astar_tests(num_tests=1000):
    """Run A* tests multiple times"""
    print(f"\n{'='*70}")
    print(f"🎯 RUNNING A* ALGORITHM TESTS ({num_tests} iterations)")
    print(f"{'='*70}\n")
    
    results = []
    failed_words = []
    
    for i in tqdm(range(num_tests), desc="Testing A*", unit="test"):
        result = test_astar_single()
...
def save_results(results, stats, output_dir='astar_test_results'):
    ...
    csv_path = os.path.join(output_dir, f'astar_results_{timestamp}.csv')
    ...
    json_path = os.path.join(output_dir, f'astar_stats_{timestamp}.json')
...
def print_statistics(stats):
    ...
    print(f"\n{'='*70}")
    print(f"📊 A* ALGORITHM STATISTICS")
    print(f"{'='*70}\n")
...
def generate_visualizations(results, stats, output_dir='astar_test_results'):
    ...
    bars = ax.bar(counts.keys(), counts.values(), color='#87CEEB', 
    ...
    ax.set_title('A* Algorithm - Guess Distribution (1000 Tests)', 
    ...
    ax.set_ylabel('Frequency', fontsize=13, fontweight='bold')
    ax.set_title('A* Algorithm - Expanded Nodes Distribution', 
                 fontsize=15, fontweight='bold', pad=15)
    ...
    ax.set_ylabel('Frequency', fontsize=13, fontweight='bold')
    ax.set_title('A* Algorithm - Execution Time Distribution', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    ...
    ax.set_ylabel('Frequency', fontsize=13, fontweight='bold')
    ax.set_title('A* Algorithm - Memory Usage Distribution', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    ...
    bp1['boxes'][0].set_facecolor('#87CEEB')
    ...
    axes[0].set_xticklabels(['A*'], fontsize=11)
    ...
    axes[1].set_xticklabels(['A*'], fontsize=11)
    ...
    axes[2].set_xticklabels(['A*'], fontsize=11)
    ...
    axes[3].set_xticklabels(['A*'], fontsize=11)
    ...
    ax.scatter(guesses, nodes, alpha=0.5, s=50, c='#87CEEB', 
    ...
    ax.set_title('A* Algorithm - Expanded Nodes vs Guesses', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ...
    ax.set_title('A* Algorithm - Execution Time vs Guesses', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    ...
    plt.title('A* Algorithm - Summary Statistics', 
...
def main():
    ...
    print("\n" + "="*70)
    print("🎯 A* ALGORITHM COMPREHENSIVE TESTING")
    print("="*70)
    ...
    # Run tests
    results, failed = run_astar_tests(num_tests=1000)

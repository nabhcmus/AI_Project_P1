"""
BFS Algorithm Comprehensive Testing & Visualization
Tests BFS algorithm 1000 times and generates detailed statistics with visualizations
"""
...
def test_bfs_single(target_word=None):
    """Test BFS algorithm for a single word"""
    try:
        word_api = TestWordAPI(5, target_word)
        solver = BFSSolver(word_api)
        
        start_time = time.time()
        # BFS needs board_state (empty list for fresh start)
        result = solver.solve(board_state=[])
        elapsed = time.time() - start_time
        
        # BFS solver returns list of path (not dict)
...
def run_bfs_tests(num_tests=1000):
    """Run BFS tests multiple times"""
    print(f"\n{'='*70}")
    print(f"🎯 RUNNING BFS ALGORITHM TESTS ({num_tests} iterations)")
    print(f"{'='*70}\n")
    
    results = []
    failed_words = []
    
    for i in tqdm(range(num_tests), desc="Testing BFS", unit="test"):
        result = test_bfs_single()
...
def save_results(results, stats, output_dir='bfs_test_results'):
    ...
    csv_path = os.path.join(output_dir, f'bfs_results_{timestamp}.csv')
    ...
    json_path = os.path.join(output_dir, f'bfs_stats_{timestamp}.json')
...
def print_statistics(stats):
    ...
    print(f"\n{'='*70}")
    print(f"📊 BFS ALGORITHM STATISTICS")
    print(f"{'='*70}\n")
...
def generate_visualizations(results, stats, output_dir='bfs_test_results'):
    ...
    bars = ax.bar(counts.keys(), counts.values(), color='#90EE90', 
    ...
    ax.set_title('BFS Algorithm - Guess Distribution (1000 Tests)', 
    ...
    ax.hist(nodes, bins=30, color='#90EE90', edgecolor='black', alpha=0.7)
    ...
    ax.set_ylabel('Frequency', fontsize=13, fontweight='bold')
    ax.set_title('BFS Algorithm - Expanded Nodes Distribution', 
                 fontsize=15, fontweight='bold', pad=15)
    ...
    ax.hist(times, bins=30, color='#FFB6C1', edgecolor='black', alpha=0.7)
    ...
    ax.set_ylabel('Frequency', fontsize=13, fontweight='bold')
    ax.set_title('BFS Algorithm - Execution Time Distribution', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    ...
    ax.set_ylabel('Frequency', fontsize=13, fontweight='bold')
    ax.set_title('BFS Algorithm - Memory Usage Distribution', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    ...
    axes[0].set_xticklabels(['BFS'], fontsize=11)
    ...
    axes[1].set_xticklabels(['BFS'], fontsize=11)
    axes[1].grid(True, alpha=0.3, axis='y')
    
    # Time box plot
    bp3 = axes[2].boxplot([times], patch_artist=True, showmeans=True,
                           meanline=True, widths=0.5)
    bp3['boxes'][0].set_facecolor('#FFB6C1')
    ...
    axes[2].set_xticklabels(['BFS'], fontsize=11)
    ...
    axes[3].set_xticklabels(['BFS'], fontsize=11)
    axes[3].grid(True, alpha=0.3, axis='y')
    
    plt.tight_layout()
    path5 = os.path.join(output_dir, f'boxplots_comparison_{timestamp}.png')
    ...
    ax.scatter(guesses, nodes, alpha=0.5, s=50, c='#90EE90', 
    ...
    ax.set_title('BFS Algorithm - Expanded Nodes vs Guesses', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    path6 = os.path.join(output_dir, f'nodes_vs_guesses_{timestamp}.png')
    ...
    ax.scatter(guesses, times, alpha=0.5, s=50, c='#FFB6C1',
               edgecolors='black', linewidth=0.5)
    ...
    ax.set_title('BFS Algorithm - Execution Time vs Guesses', 
                 fontsize=15, fontweight='bold', pad=15)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    path7 = os.path.join(output_dir, f'time_vs_guesses_{timestamp}.png')
    ...
    for i in [1, 6, 11, 16, 20]:
        table[(i, 0)].set_facecolor('#E7E6E6')
        table[(i, 1)].set_facecolor('#E7E6E6')
    
    # Style data rows
    for i in range(2, len(table_data)):
        if i not in [1, 6, 11, 16, 20]:
            if i % 2 == 0:
                table[(i, 0)].set_facecolor('#F2F2F2')
                table[(i, 1)].set_facecolor('#F2F2F2')
    
    plt.title('BFS Algorithm - Summary Statistics', 
              fontsize=16, fontweight='bold', pad=20)
    
    plt.tight_layout()
    path8 = os.path.join(output_dir, f'summary_table_{timestamp}.png')
...
def main():
    ...
    print("\n" + "="*70)
    print("🎯 BFS ALGORITHM COMPREHENSIVE TESTING")
    print("="*70)
    ...
    # Run tests
    results, failed = run_bfs_tests(num_tests=1000)

...
class Settings:
    BG = "#171717"
    SOLVE_METHODS = ["BFS", "DFS", "Entropy", "A*"]
    ...
    def __init__(self, functions):
        ...
        # ==== UI Position ====
        x_co = int(self.functions.width / 2 - (500 / 2)) + self.functions.x_co
        y_co = self.functions.y_co + int(self.functions.height / 2 - (300 / 2))
        self.window.geometry(f"500x300+{x_co}+{y_co}")
        ...
        # --------------------------
        # ENTROPY HARD MODE TOGGLE
        # --------------------------
        hard_frame = tk.Frame(self.window, background=self.BG)
        hard_frame.pack(pady=6)

        tk.Label(hard_frame, text="Entropy Hard Mode", background=self.BG,
             font="cambria 12 bold", fg="#14f41f").grid(row=0, column=0, padx=10)

        self.hard_mode_var = tk.IntVar(value=1)
        hard_check = tk.Checkbutton(hard_frame, text="Hard Mode", variable=self.hard_mode_var,
                        background=self.BG, fg="white", selectcolor="#171717", activebackground=self.BG)
        hard_check.grid(row=0, column=1, padx=8)
    ...
    def get_current_db(self):
        ...
        # Set hard mode checkbox (fallback to True if missing)
        try:
            self.hard_mode_var.set(data[0][4])
        except Exception:
            self.hard_mode_var.set(1)
    ...
    def change_db(self):
        ...
        # Update hard_mode
        cursor.execute("UPDATE info SET hard_mode=? WHERE id=0", (self.hard_mode_var.get(),))
        ...
        # Update hard_mode in main
        self.functions.hard_mode = bool(self.hard_mode_var.get())
